...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="训练 B-only 1280 集装箱编号区域检测模型")
    parser.add_argument("--model", default="yolo11m.pt", help="YOLO 预训练权重路径或模型名")
    parser.add_argument("--data", default=str(repo_root / "datasets" / "container_v1i_yolov11_b" / "data_fixed.yaml"), help="YOLO data.yaml 路径")
    parser.add_argument("--project", default=str(repo_root / "runs"), help="训练输出目录")
    parser.add_argument("--name", default="region_b_yolo11m_1280_100e", help="实验名称")
    parser.add_argument("--epochs", type=int, default=100, help="训练轮数")
    parser.add_argument("--imgsz", type=int, default=1280, help="输入尺寸")
